...
const calculateWinner = (playerCombos: number[]): boolean | number[] => {
...
const Board: FC<Props> = ({currentValue = 'o', toggleMove}) => {
  ...
  return (
    <>
      <p className={styles.title}> {winner ? 'Winner is' : 'Current move'}: {currentValue}</p>
      {winner && <button className={styles.button}>Retry</button>}
      <div className={styles.container}>
        {new Array(defaultSize * defaultSize).fill(null).map((_, index) => (
          <Cell currentValue={currentValue} onCellClicked={() => onCellClicked(index + 1)}/>
        ))}
      </div>
    </>
  );
